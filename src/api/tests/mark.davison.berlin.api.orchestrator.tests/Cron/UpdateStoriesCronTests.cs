...
public sealed class UpdateStoriesCronTests
{
    ...
    public UpdateStoriesCronTests()
    {
        ...
        _cron = new UpdateStoriesCron(
            new ScheduleConfig<UpdateStoriesCron>
            {
                CronExpression = "* * * * *",
    ...
    public async Task DoWork_CreatesJobRecord()
    {
        ...
        await Assert.That(job.JobType).Contains(nameof(UpdateStoriesRequest));
        await Assert.That(job.JobRequest).Contains(_appSettings.STORIES_PER_CRON_UPDATE.ToString());
    }
