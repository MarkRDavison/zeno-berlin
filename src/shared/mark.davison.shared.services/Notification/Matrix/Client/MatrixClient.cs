...
public class MatrixClient : IMatrixClient
{
    ...
    public async Task<Response> SendMessage(string roomId, string message)
    {
        return await SendMessageInternal(roomId, message, false);
    }

    private async Task<Response> SendMessageInternal(string roomId, string message, bool retry)
    {
        ...
        var body = new MessageBody(new TextMessageBody(message));
